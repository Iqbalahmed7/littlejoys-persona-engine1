...
def _rows_from_results(results: dict[str, dict[str, Any]] | list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not results:
        return []
...
def compute_variable_importance(results: dict[str, dict[str, Any]] | list[dict[str, Any]]) -> list[VariableImportance]:
    ...
    for row in rows:
        ...
        x_list.append(
            [
                float(row.get(feature_name, 0.0) or 0.0)
...
def generate_causal_statements(
    ...
    sorted_importances = sorted(importances, key=lambda item: item.shap_mean_abs, reverse=True)[:top_n]
    ...
    for importance in sorted_importances:
        ...
        above_rows = [row for row in variable_rows if float(row[importance.variable_name]) >= threshold]
        below_rows = [row for row in variable_rows if float(row[importance.variable_name]) < threshold]
        ...
        for segment_key in CAUSAL_SEGMENT_KEYS:
            ...
            for segment_value in segment_values:
                ...
                if not segment_above or not segment_below:
                    continue
                segment_above_rate = _adoption_rate(segment_above)
                segment_below_rate = _adoption_rate(segment_below)
                segment_lifts.append(
                    (
                        segment_value,
                        _lift_ratio(segment_above_rate, segment_below_rate),
                        segment_above_rate,
                        segment_below_rate,
                    )
                )
            ...
            if len(finite_lifts) < 2:
                continue
            if max(finite_lifts) / max(min(finite_lifts), 1e-6) <= CAUSAL_SEGMENT_LIFT_VARIATION_THRESHOLD:
                continue
