...
def _section_failed(exc: BaseException) -> None:
    st.warning(f"⚠️ This section could not load: {exc}")
    ...
    try:
                    ...
                    [
                        ...
                        for metric, a, b, delta in [
                            ...
                            (
                                ...
                                active_b,
                                (
                                    f"{active_delta_pp:+.1f}pp"
                                    if active_delta_pp is not None
                                    else "—"
                                ),
                            ),
                            (
        ...
        try:
            ...
            if scenario_a.mode == "temporal" and scenario_b.mode == "temporal":
                monthly_a = comp.temporal_monthly_a or []
                monthly_b = comp.temporal_monthly_b or []
                months_a, retention_a = _retention_curve_from_monthly(monthly_a)
                months_b, retention_b = _retention_curve_from_monthly(monthly_b)
                fig = go.Figure()
                fig.add_trace(
                    go.Scatter(
                        x=months_a,
